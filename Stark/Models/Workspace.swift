...
class Workspace: NSObject {
  ...
  func observeActivationPolicy(_ process: Process) {
    ...
    Logger.main.debug("adding observer for activation policy \(process, privacy: .public)")
  ...
  func unobserveActivationPolicy(_ process: Process) {
    ...
    if activationPolicyObserved.contains(where: { $0 == process.psn.lowLongOfPSN }) {
      ...
      Logger.main.debug("removing observer for activation policy \(process, privacy: .public)")
  ...
  func observeFinishedLaunching(_ process: Process) {
    ...
    Logger.main.debug("adding observer for finished launching \(process, privacy: .public)")
  ...
  func unobserveFinishedLaunching(_ process: Process) {
    ...
    if finishedLaunchingObserved.contains(where: { $0 == process.psn.lowLongOfPSN }) {
      ...
      Logger.main.debug("removing observer for finished launching \(process, privacy: .public)")
  ...
  override func observeValue(
    ...
    if keyPath == "activationPolicy" {
      guard let result = change?[.newKey] as? NSApplication.ActivationPolicy else { return }

      if result != process.policy {
        unobserveActivationPolicy(process)
        EventManager.shared.post(event: .applicationLaunched, object: nil)
      }
    }
